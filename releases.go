...
func showReleaseInfo(c *gin.Context) {
	...
	//k8s cluster auth params
	endpoint := c.Query("endpoint")
	token := c.Query("token")
	kubeUserName := c.Query("kubeUserName")
	kubePassword := c.Query("kubePassword")
	...
	actionConfig, err := actionConfigInit(InitKubeAllInformation(namespace, kubeContext, endpoint, token, kubeUserName, kubePassword))
	if err != nil {
		respErr(c, err)
		return
	}
